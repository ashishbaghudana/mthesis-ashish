...
public class Sentence {
	...
	public ArrayList<Phrase> getProteinTokens() {
		...
		for (Token word : words) {
			if (word.getTokenTag().equals(Constants.B_PROTEIN)) {
				...
				if (phrase!=null && !phrase.isContainedIn(proteinTokens)) {
					proteinTokens.add(phrase);
				}
				phrase = null;
			}
	...
	public ArrayList<Phrase> getRNATokens() {
		...
		for (Token word : words) {
			if (word.getTokenTag().equals(Constants.B_RNA)) {
				...
				if (phrase!=null && !phrase.isContainedIn(RNATokens))
					RNATokens.add(phrase);
				phrase = null;
			}
		}
	...
	public ArrayList<Phrase> getDNATokens() {
		...
		for (Token word : words) {
			if (word.getTokenTag().equals(Constants.B_DNA)) {
				...
				if (phrase!=null && !phrase.isContainedIn(DNATokens))
					DNATokens.add(phrase);
				phrase = null;
			}
		}
	...
	public ArrayList<Token> getTriggerWords() {
		...
		try {
			modelIn = new FileInputStream("src/main/resources/postag/en-pos-maxent.bin");
		...
		int i = 0;
		for (String tag : tags) {
			if ((tag.startsWith("VB") || tag.startsWith("NN")) && TriggerWords.triggers.contains(stemmer.stem(this.words.get(i).tokenText))) {
